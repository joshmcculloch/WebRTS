...
exports.Zombie = class extends Engine.GameObject {
    constructor (engine,location, player) {
        super (engine, "zombie",location, true);
        this.engine_id=-1;
        ...
        this.player = player;
        this.velocity = $V([this.speed,0,0]);
    ...
    update (delta_time) {
        ...
        var player_distance = this.location.distanceFrom(this.player.location);
        ...
        if (this.can_see_player) {
          this.velocity = this.player.location.subtract(this.location).toUnitVector().x(this.speed*2)
        }
        ...
        if (this.location.e(1) > 4000 || this.dead) {
            this.dead = false;
            this.respawn();
        }
        return true;
